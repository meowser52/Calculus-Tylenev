...
\end{theorem}
\begin{proof}
    Тот факт, что $\exists f^{-1}$ и она имеет тот же характер монотонности, что $f$, вытекает из предыдущей леммы.
    
    $f([a, b]) = [m, M]$ следует из \hyperlink{thm5.4}{теоремы Больцано-Коши}.

    Остаётся показать непрерывнасть $f^{-1}$ в любой точке $y \in [m, M]$. Рассмотрим случай $y_{0} \in \Bigl(m, M \Bigr)$
